...
<div wire:key="{{ $todo->id }}" class="todo mb-5 card px-5 py-6 bg-white col-span-1 border-t-2 border-blue-500 hover:shadow">
    <div class="flex justify-between space-x-2">
        ...
        <div class="flex space-x-2">
            <input wire:click="toggleChecked({{ $todo->id }})" type="checkbox" @if($todo->completed) checked @endif>


            @if( $TODO_ID_EDIT === $todo->id)
                <input type="text" placeholder="Todo.." wire:model="todo_edit_name"
                    class="bg-gray-100  text-gray-900 text-sm rounded block w-full p-2.5">


                    <span class="text-red-500 text-xs block" wire:dirty wire:target="todo_edit_name">Đã chỉnh sửa nhưng chưa được lưu</span>
                    @error('todo_edit_name')
        ...
        <div class="flex items-center space-x-2">
            ...
            <button wire:click="remove({{$todo->id}})" class="text-sm text-red-500 font-semibold rounded hover:text-teal-800 mr-1">
                <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5"
                     stroke="currentColor" class="w-4 h-4">
                    <path stroke-linecap="round" stroke-linejoin="round"
